...
Events:Subscribe('MapLoader-Villa:LoadLevel', function(saveFile)
	preset = saveFile
	CustomLevelData = preset
	print("Got savefile: " .. preset.header.projectName)
end)

NetEvents:Subscribe('MapLoader-Villa:GetLevel', function(player)
	print('Sending level to ' .. player.name)
	NetEvents:SendTo('MapLoader-Villa:GetLevel', player, CustomLevelData)
end)
